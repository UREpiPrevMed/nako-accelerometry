...
do
    ...
    then
      ...
      if [[ $CHECK -lt 6 ]]; then # check whether folder is smaller than 6 GB
         echo "... smaller than 6GB, so run pipeline"
        ...
        qsub -N "run_pipeline_200k${i}" \
          ...
          -v otd=$otd,rd=$rd $rootdir/express_run_ggir.pbs
